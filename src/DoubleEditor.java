...
public class DoubleEditor extends DefaultCellEditor {
	...
	protected boolean userSaysRevert() {
		...
		ftf.selectAll();
		Object[] options = { "Изменить", "Отменить" };
		int answer = JOptionPane.showOptionDialog(SwingUtilities.getWindowAncestor(ftf),
				"Значение может быть только между " + minimum + " и " + maximum + ".\n" + "Вы можете изменить значение "
						+ "или вернуть предыдущее значение",
				"Ошибка ввода", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE, null, options, options[1]);
